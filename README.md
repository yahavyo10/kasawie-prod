...
**Visual action timeline**: below the feed/result panels, a swimlane chart plots every
distinct action against the device it happened on — one row per device, ordered top-to-bottom
by which device the attacker touched first, so the chart reads as the actual order of
movement through the network. Markers are colored by severity (critical/error/warning/info)
so escalation is visible at a glance, sized and numbered when a burst of near-identical
actions got collapsed into one marker, and hoverable for the full message text and time
range. This is built entirely from data already computed for the LLM prompt (see
`attacker_history.condense_history`) — no extra backend work, just exposed as its own SSE
event (`timeline`) so the frontend can render it before the LLM call even finishes.
...
top right — it should say `sherlog-v22-2026-08-03-step1b-removed-again`. If it says something else, or the
...
5. Once deployed, open the Render-provided URL — same UI, same version pill check
   applies (`sherlog-v22-2026-08-03-step1b-removed-again`).
