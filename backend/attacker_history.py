...
def condense_history(entries: List[LogEntry], max_groups_per_device: int = 6, max_total_groups: int = 60) -> List[Dict[str, Any]]:
    ...
    history the user uploads. Also carries severity and device_name -- not
    needed for the LLM prompt, but needed for the visual timeline (severity
    drives marker color, device_name drives the swimlane label)."""
    ...
    for e in entries:
        ...
        if e.device_name and device_key not in device_names:
            device_names[device_key] = e.device_name
        msg_key = e.message
    ...
    for device_key, groups in by_device.items():
        ...
        for g in device_groups[:max_groups_per_device]:
            g["device_name"] = device_names.get(device_key, device_key)
