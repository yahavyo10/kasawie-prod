...
- **Phase 2** — given the 2 windows before the anomaly, build a device-scoped timeline,
  ...
  narrative renders directly below the identified IP.

Every step streams to the frontend live as it completes.

## Network inventory / CVE exposure check (Step 1b, automatic)

Once phase 1's root cause + phase 2's narrative pinpoint a CVE for the incident
(`cited_cves`), this automatically fetches your ENTIRE network inventory and asks the
LLM to reason about which OTHER devices could be affected by that CVE, using the model's
own knowledge of what it affects — not a local keyword match. Results show in the "Step
1b" panel right after Step 1. There's no UI to configure the connection — it's set once
in `.env`:

```
INVENTORY_DB_URL=postgresql://user:password@host:5432/dbname
INVENTORY_DB_SCHEMA=kasawie
INVENTORY_DB_TABLE=synthetic_network_inventory
```

This is entirely optional — leave `INVENTORY_DB_URL` unset and everything else works
exactly as before (`GET /api/health` reports `inventory_configured: false`). It also only
runs if a CVE was actually pinpointed — a purely pattern-based incident with no CVE
produces nothing to check inventory against, by design.

**How the matching actually works**: the full inventory (fetched via a plain `SELECT *` —
no assumptions about column names beyond the table itself) is first grouped by *product
signature* — vendor/model/firmware/device-type columns, auto-detected by column name —
before anything goes to the LLM. This matters at real scale: a fleet of 4,500 devices
might only have ~50 distinct signatures, so the LLM reasons about "which of these 50
products does this CVE affect" (a few thousand tokens) rather than being handed all
4,500 individual devices (tested against a real 4,500-row inventory: the naive per-device
approach would have needed ~360,000 tokens in one call — about 45x a typical free-tier
budget, an instant failure). The LLM only ever names which signature *number* (from a
numbered list) is affected; expansion back to the real device list and counts happens
deterministically in code, so the model can never invent a device that isn't actually in
your inventory. Every result includes a `caveats` field stating this is a best-effort
assessment based on the model's training data, not a substitute for the vendor's advisory
or a real vulnerability scanner. The root-cause device is excluded from the inventory
before grouping even happens, so it can never re-appear as a "newly discovered" exposure.

**Uses psycopg3, not psycopg2** — specifically to avoid the "needs a C compiler and
Postgres headers to build from source" failure mode that `psycopg2-binary` can hit on
newer Python versions / Apple Silicon. If `pip install` ever fails on this dependency,
that's the thing to check first.

**Render's internal vs. external connection string**: if your Postgres is hosted on
Render, make sure `INVENTORY_DB_URL` uses the *external* connection string (the one
ending in `.oregon-postgres.render.com` or similar), not the *internal* one — the
internal hostname only resolves from services running inside Render's own network, and
will fail with "Name or service not known" from your laptop or any other external host.

**Data-handling note worth knowing**: this sends your real inventory (hostnames, IPs,
vendor/model/firmware info) to Groq's API as part of the prompt — a genuine tradeoff for
the flexibility of not needing to hand-curate a CVE→product mapping. If that's not
acceptable for your environment, leave `INVENTORY_DB_URL` unset.

**Security note**: the connection string itself is a live, privileged credential sitting
in `.env` — same caution as the Groq key: don't commit it to a public repo (`.gitignore`
already excludes `.env`), and use a read-only/least-privilege DB user if you can, since
the app only ever needs `SELECT` access.
...
top right — it should say `sherlog-v20-2026-08-03-inventory-rebuilt`. If it says something else, or the
...
backend/
  ...
  inventory_db.py   Postgres inventory fetch + product-signature grouping for exposure scanning
...
5. Once deployed, open the Render-provided URL — same UI, same version pill check
   applies (`sherlog-v20-2026-08-03-inventory-rebuilt`).
